...
| 역할 | 이름 | 담당 영역 |
| :---: | :---: | :--- |
| 🎯 팀장 (Full-Stack) | 임재혁 | 회원 기능 총괄, 마이페이지 구현 |
| 💻 팀원 (Full-Stack) | **김민준** | **퀴즈 시스템 설계, 하트(기회) 로직, 관리자 대시보드, 퀴즈/리뷰 신고 기능, 실시간 랭킹 시스템** |
| 🎨 팀원 (Full-Stack) | 김수연 | 리뷰 시스템 개발, 유저 신뢰도 알고리즘 구현 |
| 🔧 팀원 (Full-Stack) | 동방건우 | 포인트 시스템 및 전환/관리 기능 구현 |
| 🌐 팀원 (Full-Stack) | 백승연 | 외부 영화 API(TMDB) 연동, 게시판 기능 구현 |
